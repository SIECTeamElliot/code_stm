...
*/
#define FRONT_MOTOR_SPEED_L               0.3 //0.8
...
int neutral_position(void) //88 full right | 112 neutral position | 138 full left  
{
	int born_inf = 111;
	int born_sup = 113;
	int adc_position_vol;
	//adc_position_vol = Direction_get();
	do{
		adc_position_vol = Direction_get();
		
		if(adc_position_vol > born_inf)
		{
			FrontMotor_turn(RIGHT);
		}
		if(adc_position_vol < born_sup)
		{
			FrontMotor_turn(LEFT);
		}	
		
	}while (adc_position_vol < born_inf || adc_position_vol > born_sup);
...
void regulation_pos_front(char dir) //88 full right | 112 neutral position | 138 full left  
{
//	int born_inf = 111;
//	int born_sup = 113;
	...
	}while (adc_position_vol < dir -4  || adc_position_vol > dir+4 ); //adc_position_vol != dir);
