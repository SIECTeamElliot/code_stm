...
void CAN_QuickInit(void) {
	...
	*/
	...
	CAN_InitStructure.CAN_Prescaler =6; // Prescaler for 360 000
...
//		paquet.byteMessage[4] = pDataITF_STM->mv_done;
...
void CAN_Rx_Callback(uint8_t size, int id, char * data) {
		...
		else if (id == ID_MANOEUVRE_VOIT) {
			pDataITF_PI->m_voiture = paquet.byteMessage[0];	
		}		
